...
#   Github - sprov-ui: https://github.com/liu060011/sprov-ui
#=================================================
...
install_sprov-ui() {
    ...
    last_version=$(curl --silent "https://api.github.com/repos/liu060011/sprov-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    echo -e "检测到sprov-ui最新版本：${last_version}，开始下载核心文件"
    wget -N --no-check-certificate -O /usr/local/sprov-ui/sprov-ui.jar https://github.com/liu060011/sprov-ui/raw/master/sprov-ui.jar
    if [[ $? -ne 0 ]]; then
    ...
    echo -e "wget -O /usr/bin/sprov-ui -N --no-check-certificate https://github.com/liu060011/sprov-ui/raw/master/sprov-ui.sh && chmod +x /usr/bin/sprov-ui"
    echo -e ""
    echo -e "若未安装 bbr 等加速工具，推荐使用以下命令一键安装 bbr："
    echo -e "wget --no-check-certificate https://github.com/liu060011/sprov-ui/raw/master/bbr.sh && bash bbr.sh"
    echo -e ""
}
