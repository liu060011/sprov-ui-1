...
install() {
    install_base
    bash <(curl -L -s https://github.com/liu060011/sprov-ui/raw/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
uninstall() {
    ...
    echo -e "Github issues: ${green}https://github.com/liu060011/sprov-ui/issues${plain}"
...
install_bbr() {
    bash <(curl -L -s https://github.com/liu060011/sprov-ui/raw/master/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/sprov-ui -N --no-check-certificate https://github.com/liu060011/sprov-ui/raw/master/sprov-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
